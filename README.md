# Clasificacion-de-galaxias-por-morfologia-
...
Para este clasificador, no empece desde cero. Se implemento un Transfer Learning utilizando la arquitectura MobileNetV2 como base.

¿Por qué MobileNetV2? En la necesidad de un modelo que fuera eficiente pero potente, al estar pre-entrenado en millones de imágenes (ImageNet), el modelo ya "sabe" detectar bordes, texturas y formas básicas.
Ademas al usar MobileNetV2, se tiene una arquitectura de 52 capas con pesos entrenables y el número de filtros va aumentando a medida que la red profundiza, comenzando en 32 y llegando hasta 1280 filtros en la última capa de extracción de características.
...
A pesar de una precisión general aceptable, el análisis del Recall revela un sesgo hacia las galaxias elípticas. El bajo F1-Score en las clases Espiral y Lenticular sugiere que la ambigüedad visual de estas categorías actúa como un cuello de botella. Esto indica que el modelo es conservador: prefiere clasificar objetos dudosos como 'lisos' (Elípticas) en lugar de asignar estructuras que no detecta con total certeza
